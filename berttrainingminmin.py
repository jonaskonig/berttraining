...
push_to_hub_callback = PushToHubCallback(
    ...
    output_dir="./huggingfacedest23", tokenizer=tokenizer, hub_model_id="jonaskoenig/xtremedistil-l6-h256-uncased-future-time-references-D2",
...
dataset = load_dataset("jonaskoenig/future-time-refernces-static-filter-D2")
datasettrump = load_dataset("jonaskoenig/trump_administration_statement")
datasettrump.set_format(type="pandas")
dataset.set_format(type="pandas")
trumptrain = datasettrump["train"][:]
dattrain = dataset["train"][:]
trumptest = datasettrump["test"][:]
dattest  = dataset["test"][:]
trumpval = datasettrump["validation"][:]
datval = dataset["validation"][:]
datasettrain = Dataset.from_pandas(pd.concat([trumptrain,dattrain]))
datasettest = Dataset.from_pandas(pd.concat([trumptest,dattest]))
datasetval = Dataset.from_pandas(pd.concat([trumpval,datval]))
#print(datasettrain)
...
model.fit(tf_train_dataset, validation_data=tf_validation_dataset, epochs=5, batch_size=BATCHSIZE,
          callbacks=[push_to_hub_callback, tensorboard])
_, accuracy = model.evaluate(tf_test_dataset)
print(f"Test accuracy: {round(accuracy * 100, 2)}%")

model.push_to_hub("jonaskoenig/xtremedistil-l6-h256-uncased-future-time-references-D2",
                  use_auth_token="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")
tokenizer.push_to_hub('jonaskoenig/xtremedistil-l6-h256-uncased-future-time-references-D2',
                      use_auth_token="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")
